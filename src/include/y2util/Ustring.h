...
/**
 * @short A Hash of unique std::strings.
 *
 * A Hash of unique std::strings.
 *
 * @see Ustring
 **/
class UstringHash {
  ...
  protected:
     ...
     * comparing the stored std::string. That way existing strings
     * can be found by hash functions.
     *
     * @see UstringHash
     **/
  ...
  public:
    ...
    /**
     * Store the passed in the hash, if it does not already contain a string
     * with the same value. Return the unique string from the hash.
     **/
...
//	CLASS NAME : Ustring
/**
 * @short Base class for unique std::strings
 *
 * The std::string stored inside Ustring shares his representaion
 * class (which contains the actual data) with a unique std::string
 * stored inside a UstringHash.
 *
 * That way Ustrings of the same value can be stored at various locations,
 * while the actual data are stored only once.
 *
 * <PRE>
